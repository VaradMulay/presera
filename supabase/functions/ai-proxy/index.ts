...
// (OpenRouter) server-side; the result flows back. The frontend never
// touches an AI provider directly.
//
// Architecture:
//   Presera frontend → this Edge Function → OpenRouter → back to frontend
//
// Authentication:
...
//
// AI secrets:
//   The OpenRouter API key is stored as a Supabase Edge Function secret
//   (OPENROUTER_API_KEY) and read via Deno.env.get("OPENROUTER_API_KEY").
...
//   - "evaluateAnswer"      → evaluate a single answer
//   - "generateEvaluation"  → generate final interview evaluation
//
...
function getAiProviderKey(): string | null {
  ...
  try {
    response = await fetch(OPENROUTER_ENDPOINT, {
      ...
      headers: {
        ...
        "HTTP-Referer": "https://presera.app",
...
function buildQuestionPrompt(p: GenerateQuestionsPayload): string {
  ...
  const hasResume = !!(p.resumeText && p.resumeText.trim());
  if (hasResume) {
    contextParts.push(`CANDIDATE RESUME (plain text extracted from uploaded file):\n${p.resumeText.trim()}`);
  }
  ...
  // Resume-specific instruction: force the model to mine the resume for
  // concrete anchors and weave them into the questions.
  const resumeInstruction = hasResume
    ? `\nIMPORTANT — RESUME-BASED QUESTIONS ARE MANDATORY:
Analyze the candidate's resume text above. You MUST generate questions that directly reference the candidate's SPECIFIC past projects, the technologies/tools/frameworks in their technical stack, and concrete experience described in the resume.
- Identify named projects, systems, or products from the resume and ask the candidate to explain architectural decisions, trade-offs, or challenges they faced in those specific projects.
- Reference the exact technologies, frameworks, and tools listed in the resume (e.g. "You mentioned using React and Redux in your checkout-service project — how did you handle state synchronization…").
- Probe specific role responsibilities, scale metrics, or achievements the resume claims (e.g. "Your resume says you reduced API latency by 40% — walk me through how you identified and resolved the bottleneck").
- At least half of the questions should be anchored in specific resume content. The rest should cover the target role's core competencies.
- Do NOT ask generic questions that could apply to any candidate. Every resume-anchored question must mention a specific detail from the resume text.`
    : "\nNo resume was provided. Generate questions based solely on the target role, difficulty, and job description (if provided). Cover the core competencies and common scenarios for this role.";
...
${categoryGuidance}${contextBlock}${resumeInstruction}
...
Return a JSON object with a single key "questions" whose value is a JSON array of exactly ${p.numberOfQuestions} objects with this shape:
{
...
function parseQuestions(
  ...
  // The prompt asks for { "questions": [...] }, but some models may return
