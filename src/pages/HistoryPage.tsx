  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-8 flex flex-col gap-4 animate-fade-in sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white">Interview History</h1>
      ...
      {completedCount === 0 ? (
        ...
        <>
          {/* Filters */}
          <Card className="mb-6 animate-fade-in" padded={false}>
            <div className="flex flex-col gap-3 p-4 lg:flex-row lg:items-center">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-xl border border-white/15 bg-white/5 backdrop-blur-sm pl-10 pr-3 text-sm text-white placeholder:text-slate-500 transition focus:border-brand-400 focus:bg-white/10 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                />
              </div>
...
function HistoryRow({ interview: iv }: { interview: InterviewSession }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-4 rounded-2xl border border-white/15 bg-white/5 backdrop-blur-md p-4 transition hover:-translate-y-0.5 hover:border-brand-400/30 hover:bg-brand-500/10 sm:flex-row sm:items-center sm:justify-between"
    >
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          <p className="font-semibold text-white">{iv.role}</p>
      ...
      <div className="flex items-center justify-between gap-4 sm:justify-end">
        <div className="flex items-center gap-3">
          {inProgress ? (
            <span className="rounded-full bg-warning-500/15 border border-warning-400/20 px-3 py-1 text-xs font-semibold text-warning-300">
              In progress
            </span>
          ) : (
            <div className="text-right">
              ...
              <p className="mt-1 text-[11px] font-medium text-slate-500">{scoreLabel(iv.result!.overall)}</p>
            </div>
          )}
          <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-white/15 text-slate-400 transition group-hover:border-brand-400/30 group-hover:text-brand-400">
