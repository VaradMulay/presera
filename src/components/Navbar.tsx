  ...
  return (
    <header className="sticky top-0 z-50 border-b border-neutral-200/80 bg-white/80 backdrop-blur-md">
      <nav className="container-app flex h-16 items-center justify-between">
        ...
        <button
          className="rounded-lg p-2 text-neutral-700 transition hover:bg-neutral-100 md:hidden"
          onClick={() => setOpen((v) => !v)}
          ...
          <div className="container-app flex flex-col gap-1 py-4">
            {NAV_LINKS.map((l) => (
              <a
                ...
                className={cn(
                  'rounded-lg px-3 py-2.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-100',
                  location.pathname === l.href && 'bg-neutral-100',
                )}
              >
                {l.label}
              </a>
